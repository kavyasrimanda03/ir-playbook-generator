...
def generate_pdf(playbook: dict, output_path: str = None) -> str:
    ...
    # Severity Assessment (if calculator was used)
    assessment = playbook.get("severity_assessment")
    if assessment:
        story.append(_section_table("SEVERITY ASSESSMENT RESULT", color=SEVERITY_COLORS.get(assessment["severity"], DARK)))
        # Score row
        score_data = [[
            Paragraph("<b>SCORE</b>", ParagraphStyle("sl", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)),
            Paragraph(f"{assessment['score']} / {assessment['max_score']} ({assessment['percentage']}%)",
                      ParagraphStyle("sv", fontName="Helvetica-Bold", fontSize=11, textColor=DARK)),
            Paragraph("<b>SEVERITY</b>", ParagraphStyle("sl2", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)),
            Paragraph(f"<b>{assessment['severity'].upper()}</b>",
                      ParagraphStyle("sev", fontName="Helvetica-Bold", fontSize=11,
                                     textColor=SEVERITY_COLORS.get(assessment["severity"], DARK))),
        ]]
        score_t = Table(score_data, colWidths=[30*mm, 55*mm, 30*mm, 55*mm])
        ...
        if assessment.get("top_risk_factors"):
            story.append(Paragraph("<b>TOP RISK FACTORS:</b>", ParagraphStyle("rf", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)))
            for factor in assessment["top_risk_factors"]:
                story.append(Paragraph(f"▸  {factor}", S["body"]))
        story.append(Spacer(1, 2*mm))

        if assessment.get("recommendations"):
            story.append(Paragraph("<b>IMMEDIATE RECOMMENDATIONS:</b>", ParagraphStyle("rc", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)))
    ...
    if scan:
        scan_color = SEVERITY_COLORS.get(scan["risk_level"], DARK)
        story.append(_section_table(f"LOG SCAN RESULTS — {scan['risk_level'].upper()} RISK", color=scan_color))
        scan_meta = [
            ["File Scanned", os.path.basename(scan["file"])],
            ["Lines Scanned", str(scan["lines_scanned"])],
            ["IOCs Found", str(scan["total_findings"])],
            ["Risk Score", f"{scan['risk_score']} / 100"],
            ["Risk Level", scan["risk_level"]],
            ["Attack Types Detected", ", ".join(scan["matched_incident_types"]) or "None"],
        ]
        sm_table = Table(
            [[Paragraph(f"<b>{r[0].upper()}</b>", ParagraphStyle("sml", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)),
              Paragraph(r[1], ParagraphStyle("smv", fontName="Helvetica", fontSize=9, textColor=DARK))]
             for r in scan_meta],
        ...
        if scan["findings"]:
            story.append(Paragraph("<b>IOC FINDINGS:</b>", ParagraphStyle("ioch", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)))
            story.append(Spacer(1, 1*mm))
            ioc_rows = [[
                Paragraph("<b>SEV</b>", ParagraphStyle("ih0", fontName="Helvetica-Bold", fontSize=7, textColor=colors.white)),
                Paragraph("<b>DESCRIPTION</b>", ParagraphStyle("ih1", fontName="Helvetica-Bold", fontSize=7, textColor=colors.white)),
                Paragraph("<b>LINE</b>", ParagraphStyle("ih2", fontName="Helvetica-Bold", fontSize=7, textColor=colors.white)),
                Paragraph("<b>CATEGORY</b>", ParagraphStyle("ih3", fontName="Helvetica-Bold", fontSize=7, textColor=colors.white)),
            ]]
            for f in scan["findings"][:10]:
                fc = SEVERITY_COLORS.get(f["severity"], SLATE)
                ioc_rows.append([
                    Paragraph(f"<b>{f['severity']}</b>", ParagraphStyle("fc", fontName="Helvetica-Bold", fontSize=7, textColor=colors.white)),
                    Paragraph(f["description"], ParagraphStyle("fd", fontName="Helvetica", fontSize=7, textColor=DARK)),
                    Paragraph(str(f["line_number"]), ParagraphStyle("fl", fontName="Helvetica", fontSize=7, textColor=DARK)),
                    Paragraph(f["category"], ParagraphStyle("fcat", fontName="Helvetica", fontSize=7, textColor=DARK)),
                ])
            ioc_t = Table(ioc_rows, colWidths=[18*mm, 100*mm, 15*mm, 37*mm])
            ioc_style = [
            ...
            for i, f in enumerate(scan["findings"][:10], 1):
                fc = SEVERITY_COLORS.get(f["severity"], SLATE)
                ioc_style.append(("BACKGROUND", (0,i), (0,i), fc))
                ioc_style.append(("BACKGROUND", (1,i), (-1,i), LIGHT))
            ioc_t.setStyle(TableStyle(ioc_style))
            story.append(ioc_t)

        if scan.get("recommendations"):
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph("<b>SCAN RECOMMENDATIONS:</b>", ParagraphStyle("srh", fontName="Helvetica-Bold", fontSize=8, textColor=SLATE)))
